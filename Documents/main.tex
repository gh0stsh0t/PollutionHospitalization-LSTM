...
From the research, RNNs are a type of deep neural network that uses a state cell to act as memory to use previous inputs in a sequence. This make it a good choice for timeseries data which takes into account the order of inputs
...
Using LSTM cells the model was set with grid searched parameters that uses the input data to predict the number of hospitalizations for the next week. The network can predict somewhat reliably however it perfoms poorly when input is missing a large amount of data
